...
To override the project for a specific working directory, add `DEEPAGENTS_CODE_LANGSMITH_PROJECT` to a `.env` in that directory. See [environment variables](/oss/javascript/deepagents/code/configuration#environment-variables) for the full loading order.
